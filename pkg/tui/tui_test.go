...
func TestWizardFactoryOptions(t *testing.T) {
	...
	m.state = stateOptions
	if len(m.optionsList) != 4 || len(m.optionsValues) != 4 {
		t.Fatalf("options screen has %d/%d entries, want 4", len(m.optionsList), len(m.optionsValues))
	}

	m.optionsValues[optDistinctVerifier] = true
	m.optionsValues[optEscalate] = true
	...
	m2.optionsValues[optEscalate] = false
	...
	m3.optionsValues[optEscalate] = true
...
// The agents screen commits its seats and goes straight to options — no
// intermediate screens.
func TestWizardAgentsRouteToOptions(t *testing.T) {
	cfg := config.NewDefaultConfig()
	cfg.BinaryOverrides = fakeInstalled("claude", "codex")
	m := NewWizardModel(cfg)
	m.state = stateAgents
	for _, step := range []string{"discover", "execute", "verify", "commit"} {
		m.stepToAgent[step] = "codex"
	}
	m = sendW(m, keyEnter())
	if m.state != stateOptions {
		t.Fatalf("agents commit should land on options, got %d", m.state)
	}
	if m.cfg.Execute.Agent != "codex" {
		t.Errorf("execute agent = %q, want codex", m.cfg.Execute.Agent)
	}
	// No escalation -> options goes straight to maxIters.
	m.optionsValues[optEscalate] = false
	m = sendW(m, keyEnter())
	if m.state != stateMaxIters {
		t.Fatalf("options commit (no escalation) should land on maxIters, got %d", m.state)
	}
}
