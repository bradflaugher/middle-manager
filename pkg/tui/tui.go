...
func (m *WizardModel) nextStep() (tea.Model, tea.Cmd) {
	...
	case stateConfirm:
		// Persist the durable preferences so they seed every future run — best
...
func (m *WizardModel) confirmView() string {
	...
	var modelBits []string
	for _, step := range m.agentStepKeys() {
		if sc := m.cfg.StepFor(step); sc != nil && sc.Model != "" {
			modelBits = append(modelBits, fmt.Sprintf("%s=%s", m.agentRowLabel(step), sc.Model))
		}
	}
	if len(modelBits) > 0 {
