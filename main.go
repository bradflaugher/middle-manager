...
// run's outcome, so `mm status` answers "where is my time going". A queue
// drain writes one ledger per issue under issues/<n>/, so the whole drain
// (and the base dir's single runs) roll up into one table.
func printLedgerSummary(stateDir string) {
	...
	for _, name := range names {
		a := perAgent[name]
		fmt.Printf("  %-12s %6d %9s %8d %9d %6d\n", name, a.steps, (time.Duration(a.seconds) * time.Second).Round(time.Second), a.retries, a.timeouts, a.escals)
	}
	if lastRun != nil {
		outcome := "failed"
