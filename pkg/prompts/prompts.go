...
const DiscoverRepairTemplate = `# Repair Audit — Iteration {iteration}

You are the **Auditor**. There is no issue and no feature request — your job is
to FIND the most valuable fix yourself and scope it for the programmer agent.
...
## Verifier feedback and previous errors (if any)
{error_log}

## Your job

1. **Scan the codebase for CONCRETE defects**: failing builds or tests, real
   bugs, dead links, docs that contradict the code, missing error handling,
   broken examples. Evidence over vibes — run checks where you can.
2. **Pick ONE fix**: the highest-value, smallest-scope defect. Never propose a
   rewrite or a grab-bag of unrelated cleanups.
3. **Do not modify any files** — planning and discovery only.
4. Output a self-contained brief for the programmer: the defect (exact file
   paths and evidence), the fix plan, and mechanically checkable acceptance
   criteria the verifier can enforce. Your full output is handed to the
   programmer verbatim.`
